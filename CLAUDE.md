...
- **メインカラー**: `#335BA5`（ダークモード: `#5B8BD4`）
  - CSS変数: `--brand`（globals.cssで定義。`.dark` で明るい色に自動切替）
  - Tailwindクラス: `text-brand`, `bg-brand`, `border-brand`
  - 使用箇所: ヘッダータイトル
...
│   │   ├── breakdown-charts.tsx # 収支内訳円グラフ（支出・収入の割合）
│   │   ├── milestone-card.tsx  # マイルストーンカード
...
- **右カラム**: KPIカード + グラフ + 内訳
  ...
  3. **収支内訳 + マイルストーン** (1:1:2レイアウト)
     - **支出内訳円グラフ** - 初期費用 / 固定費 / 変動費 / 決済手数料の割合
     - **収入内訳円グラフ** - サブスク / 広告の割合
     - **マイルストーンカード** - 目標到達状況（達成月を表示）
...
7. ~~**収支内訳円グラフの表示**~~ ✅ — グラフ下に支出・収入それぞれの割合を円グラフで可視化。`calculateBreakdown` 関数でカテゴリ別集計
8. ~~**マイルストーンカード**~~ ✅ — 目標到達月を表示（TODO: シミュレーションデータから動的計算）
