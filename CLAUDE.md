...
1. **損益分岐点（BEP）のハイライト表示** — 累計損益が負→正に転じる月にReferenceLineで縦線を表示。線形補間で小数月にも対応 ✅
2. **Tooltipに通貨単位（¥）を追加** — Tooltipの金額表示に¥プレフィックスを付与
3. **Y軸の大きな数値を短縮フォーマット** — `1,000,000` → `100万` のような短縮表示
4. **月次損益（profit）の折れ線を追加** — 累計損益に加えて単月損益も表示し、単月黒字化タイミングを可視化
5. **サマリー指標（KPIカード）の表示** — グラフ上部にBEP到達月、最終月累計損益、最終月ユーザー数、MRRなどを表示
