      ...
      <CardContent>
        <ul className="space-y-3">
          ...
          <li className="flex items-center gap-3">
            ...
            <span className="flex-1 text-sm">{t("endPeriodUsers")}</span>
            <span className="text-sm font-semibold tabular-nums">
              {last !== null ? last.users.toLocaleString() : "—"}
              {last !== null && t("suffix") && (
                <span className="ml-0.5 text-xs font-normal text-muted-foreground">{t("suffix")}</span>
              )}
            </span>
          </li>
          ...
          {planSubscribers.length === 0 ? (
            <li className="flex items-center gap-3">
              ...
              <span className="flex-1 text-sm">{t("endPeriodSubscribers")}</span>
